...
export default function TourPage() {
  ...
  // Fetch similar tours (same category, exclude current)
  useEffect(() => {
    const fetchSimilar = async () => {
      ...
      try {
        ...
        const similar = all
          .filter(
            (t: any) => t.category === tour.category && t._id !== tour._id,
          )
          .slice(0, 6);
        ...
        <div className="bg-muted/30 py-16">
          <div className="container mx-auto px-4 max-w-6xl">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="font-display text-2xl md:text-3xl font-bold text-foreground">
                  Similar <span className="text-primary">Tours</span>
                </h2>
                <p className="text-muted-foreground text-sm mt-1">
                  More {tour?.category} adventures you might love
                </p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => setSimilarIdx((p) => Math.max(0, p - 1))}
                  disabled={similarIdx === 0}
                  className="w-9 h-9 rounded-full border border-border bg-card flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary disabled:opacity-30 transition-all"
                >
                ...
                <button
                  onClick={() =>
                    setSimilarIdx((p) =>
                      Math.min(similarTours.length - 1, p + 1),
                    )
                  }
                  disabled={similarIdx >= similarTours.length - 1}
                  className="w-9 h-9 rounded-full border border-border bg-card flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary disabled:opacity-30 transition-all"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 overflow-hidden">
              {similarTours.slice(similarIdx, similarIdx + 3).map((t: any) => {
                ...
                return (
                  <Link
                    key={t._id}
                    href={`/tours/detail/${t._id}`}
                    className="group bg-card border border-border rounded-2xl overflow-hidden shadow-md hover:shadow-xl hover:border-primary/30 hover:-translate-y-1 transition-all duration-300 flex flex-col"
                  >
                    <div className="relative h-44 overflow-hidden bg-muted">
                      ...
                      {t.category && (
                        <span className="absolute top-3 left-3 bg-primary/90 text-white text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">
                          {t.category}
                        </span>
                      )}
                    </div>
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <div className="mt-auto pt-4 flex items-center justify-between border-t border-border">
                        <span className="font-bold text-sm text-primary">
                          {t.price ? `From $${t.price}` : "On Request"}
                        </span>
                        <span className="text-xs font-semibold text-muted-foreground flex items-center group-hover:text-primary transition-colors">
                          View Tour <ArrowRight className="w-3.5 h-3.5 ml-1" />
                        </span>
                      </div>
                    </div>
                  </Link>
