...
export default function TourPage() {
  ...
  const handleBooking = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const email =
      ...
      "";
    const pax = Number(
      (form.elements.namedItem("pax") as HTMLInputElement)?.value || 1,
    );
    const message =
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              <Badge
                variant="outline"
                className="flex items-center text-base py-1 px-3"
              >
          ...
          {tour.whyChoose?.length > 0 && (
            <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
              <h2 className="text-xl sm:text-2xl font-bold mb-5">
                Why Choose Mountain Travels Pakistan?
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {tour.whyChoose.map((item: any, i: number) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 p-4 rounded-lg bg-secondary/5 border border-secondary/10"
                  >
            ...
            <Tabs defaultValue="itinerary" className="w-full">
              <div style={{ marginBottom: "100px" }}>
                <TabsList className="grid w-full grid-cols-2 md:grid-cols-5 gap-2 mb-8">
                  {["itinerary", "inclusions", "faqs", "terms", "map"].map(
                    (tab) => (
                      <TabsTrigger
                        key={tab}
                        value={tab}
                            ...
                            {day.images?.length > 0 && (
                              <div className="relative aspect-video">
                                <Image
                                  ...
                                  alt={
                                    day.title?.trim() || `Day ${day.day} Image`
                                  }
                                  fill
              ...
              ${isAvailable ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
                    >
                      <div>
                        ...
                        <p>
                          <strong>Price:</strong> ${dep.price}
                        </p>
                        <p>
                          <strong>Spots Left:</strong> {dep.spotsLeft}
                        </p>
                      </div>
                      <span
        ...
        <DialogContent className="bg-green-100 p-6 rounded-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-lg">
              Thank you for Booking {tour.name}. We will contact you shortly
