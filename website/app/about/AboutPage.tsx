    ...
    async function fetchTestimonials() {
      try {
        ...
        if (testData.success && testData.data && testData.data.length > 0) {
          // Limit to first 4 testimonials for the about page grid
          setTestimonials(testData.data.slice(0, 4));
        } else {
          setTestimonials([]);
        }
      } catch (err) {
              ...
              <RevealStagger key={idx} delayOffset={idx * 0.05}>
                <div className="bg-background border border-border p-6 h-full flex flex-col justify-between hover:shadow-xl hover:border-primary/20 transition-all duration-300 rounded-2xl relative overflow-hidden group">
                  ...
                  <div className="mt-4 pt-4 border-t border-border relative z-10 flex items-center gap-3">
                    {quote.image ? (
                      <div className="w-9 h-9 rounded-full overflow-hidden shrink-0 shadow-inner">
                        <Image
                          src={quote.image}
                          alt={quote.name}
                          width={36}
                      ...
                      <div className="w-9 h-9 rounded-full bg-primary/15 text-primary font-bold text-xs flex shrink-0 items-center justify-center shadow-inner">
                        {quote.name.charAt(0)}
                      </div>
                    )}
                    <div>
